...
import { X, Diamond, MapPin, AlertCircle, CheckCircle2, ShoppingBag, TrendingUp } from 'lucide-react'
      ...
      >
        ...
        {item.image ? (
          <div className="w-full bg-gradient-to-br from-romantic-pink/30 to-romantic-dark/5">
            <img src={item.image} alt={item.name} className="w-full h-auto max-h-[400px] object-contain" />
          </div>
        ) : (
          <div className="w-full py-16 bg-gradient-to-br from-romantic-pink/30 to-romantic-dark/5 flex items-center justify-center">
        ...
        <div className="p-6 space-y-5">
          <div>
            <div className="flex items-start justify-between gap-3">
              <h2 className="font-cormorant text-2xl font-bold text-romantic-dark">{item.name}</h2>
              <Badge variant="default">{CATEGORY_ICONS[item.category]} {CATEGORY_LABELS[item.category]}</Badge>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex items-start gap-3 p-3 rounded-xl bg-romantic-pink/20">
              ...
              <div>
                ...
                <p className="font-nunito font-semibold text-romantic-dark">Сезон {item.season}, Серия {item.episode}</p>
              </div>
            </div>
          ...
          <div className="text-center pt-2">
            <span className="text-romantic-gold/30 text-xs font-cormorant italic">✦ Magic Wardrobe ✦</span>
          </div>
        </div>
      </motion.div>
    </motion.div>
